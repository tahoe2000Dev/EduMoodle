 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswer2 extends question_type {
    ...
    public function get_question_options($question) {
        ...
        // Get relevant data indexed by positionkey from the multianswer2s table.
        $sequence = $DB->get_field('question_multianswer2', 'sequence',
    ...
    public function save_question_options($question) {
        ...
        if (!$oldwrappedids = $DB->get_field('question_multianswer2', 'sequence',
                    ...
                    if ($oldwrappedquestion->qtype != $wrapped->qtype) {
                        switch ($oldwrappedquestion->qtype) {
                            case 'multichoice':
                                ...
                                break;
                            case 'shortanswer2':
                                $DB->delete_records('qtype_shortanswer2_options',
                                        array('questionid' => $oldwrappedquestion->id));
                                break;
                            case 'shortanswer':
                            ...
                            default:
                                throw new moodle_exception('qtypenotrecognized',
                                        'qtype_multianswer2', '', $oldwrappedquestion->qtype);
                                $wrapped->id = 0;
                        }
                    }
                } else {
        ...
        if (!empty($sequence)) {
            $multianswer2 = new stdClass();
            $multianswer2->question = $question->id;
            $multianswer2->sequence = implode(',', $sequence);
            if ($oldid = $DB->get_field('question_multianswer2', 'id',
                    array('question' => $question->id))) {
                $multianswer2->id = $oldid;
                $DB->update_record('question_multianswer2', $multianswer2);
            } else {
                $DB->insert_record('question_multianswer2', $multianswer2);
            }
        }
    ...
    public function save_question($authorizedquestion, $form) {
        $question = qtype_multianswer2_extract_question($form->questiontext);
    ...
    public function delete_question($questionid, $contextid) {
        global $DB;
        $DB->delete_records('question_multianswer2', array('question' => $questionid));
...
define('ANSWER_ALTERNATIVE_FRACTION_REGEX2',
       '=|%(-?[0-9]+)%');
// For the syntax '(?<!' see http://www.perl.com/doc/manual/html/pod/perlre.html#item_C.
define('ANSWER_ALTERNATIVE_ANSWER_REGEX2',
        '.+?(?<!\\\\|&|&amp;)(?=[~#}]|$)');
define('ANSWER_ALTERNATIVE_FEEDBACK_REGEX2',
        '.*?(?<!\\\\)(?=[~}]|$)');
define('ANSWER_ALTERNATIVE_REGEX2',
       '(' . ANSWER_ALTERNATIVE_FRACTION_REGEX2 .')?' .
       '(' . ANSWER_ALTERNATIVE_ANSWER_REGEX2 . ')' .
       '(#(' . ANSWER_ALTERNATIVE_FEEDBACK_REGEX2 .'))?');

// Parenthesis positions for ANSWER_ALTERNATIVE_REGEX.
define('ANSWER_ALTERNATIVE_REGEX_PERCENTILE_FRACTION2', 2);
define('ANSWER_ALTERNATIVE_REGEX_FRACTION2', 1);
define('ANSWER_ALTERNATIVE_REGEX_ANSWER2', 3);
define('ANSWER_ALTERNATIVE_REGEX_FEEDBACK2', 5);

// NUMBER_FORMATED_ALTERNATIVE_ANSWER_REGEX is used
// for identifying numerical answers in ANSWER_ALTERNATIVE_REGEX_ANSWER.
define('NUMBER_REGEX2',
        '-?(([0-9]+[.,]?[0-9]*|[.,][0-9]+)([eE][-+]?[0-9]+)?)');
define('NUMERICAL_ALTERNATIVE_REGEX2',
        '^(' . NUMBER_REGEX2 . ')(:' . NUMBER_REGEX2 . ')?$');

// Parenthesis positions for NUMERICAL_FORMATED_ALTERNATIVE_ANSWER_REGEX.
define('NUMERICAL_CORRECT_ANSWER2', 1);
define('NUMERICAL_ABS_ERROR_MARGIN2', 6);

// Remaining ANSWER regexes.
define('ANSWER_TYPE_DEF_REGEX2',
        '(NUMERICAL|NM)|(MULTICHOICE|MC)|(MULTICHOICE_V|MCV)|(MULTICHOICE_H|MCH)|' .
        '(SHORTANSWER2|SA2)|(SHORTANSWER2_C|MA2C)|' .
...
define('ANSWER_START_REGEX2',
       '\{([0-9]*):(' . ANSWER_TYPE_DEF_REGEX2 . '):');

define('ANSWER_REGEX2',
        ANSWER_START_REGEX2
        . '(' . ANSWER_ALTERNATIVE_REGEX2
        . '(~'
        . ANSWER_ALTERNATIVE_REGEX2
        . ')*)\}');
...
define('ANSWER_REGEX_NORM2', 1);
define('ANSWER_REGEX_ANSWER_TYPE_NUMERICAL2', 3);
define('ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE2', 4);
define('ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE_REGULAR2', 5);
define('ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE_HORIZONTAL2', 6);
define('ANSWER_REGEX_ANSWER_TYPE_SHORTANSWER22', 7);
define('ANSWER_REGEX_ANSWER_TYPE_SHORTANSWER2_C2', 8);
define('ANSWER_REGEX_ANSWER_TYPE_SHORTANSWER2', 9);
define('ANSWER_REGEX_ANSWER_TYPE_SHORTANSWER_C2', 10);
define('ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE_SHUFFLED2', 11);
define('ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE_REGULAR_SHUFFLED2', 12);
define('ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE_HORIZONTAL_SHUFFLED2', 13);
define('ANSWER_REGEX_ANSWER_TYPE_MULTIRESPONSE2', 14);
define('ANSWER_REGEX_ANSWER_TYPE_MULTIRESPONSE_HORIZONTAL2', 15);
define('ANSWER_REGEX_ANSWER_TYPE_MULTIRESPONSE_SHUFFLED2', 16);
define('ANSWER_REGEX_ANSWER_TYPE_MULTIRESPONSE_HORIZONTAL_SHUFFLED2', 17);
define('ANSWER_REGEX_ALTERNATIVES2', 18);
 ...
 *
 */
function qtype_multianswer2_initialise_multichoice_subquestion($wrapped) {
...
function qtype_multianswer2_extract_question($text) {
    // Variable $text is an array [text][format][itemid].
    $question = new stdClass();
    $question->qtype = 'multianswer2';
    ...
    for ($positionkey = 1;
            preg_match('/'.ANSWER_REGEX2.'/s', $question->questiontext['text'], $answerregs);
            ++$positionkey) {
        $wrapped = new stdClass();
        $wrapped->generalfeedback['text'] = '';
        $wrapped->generalfeedback['format'] = FORMAT_HTML;
        $wrapped->generalfeedback['itemid'] = '';
        if (isset($answerregs[ANSWER_REGEX_NORM2]) && $answerregs[ANSWER_REGEX_NORM2] !== '') {
            $wrapped->defaultmark = $answerregs[ANSWER_REGEX_NORM2];
        } else {
            $wrapped->defaultmark = '1';
        }
        if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_NUMERICAL2])) {
        ...
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_SHORTANSWER22])) {
            $wrapped->qtype = 'shortanswer2';
            $wrapped->usecase = 0;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_SHORTANSWER2_C2])) {
            $wrapped->qtype = 'shortanswer2';
            $wrapped->usecase = 1;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_SHORTANSWER2])) {
            $wrapped->qtype = 'shortanswer';
            $wrapped->usecase = 0;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_SHORTANSWER_C2])) {
            $wrapped->qtype = 'shortanswer';
            $wrapped->usecase = 1;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE2])) {
            qtype_multianswer2_initialise_multichoice_subquestion($wrapped);
            $wrapped->shuffleanswers = 0;
            $wrapped->layout = qtype_multichoice_base::LAYOUT_DROPDOWN;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE_SHUFFLED2])) {
            qtype_multianswer2_initialise_multichoice_subquestion($wrapped);
            $wrapped->shuffleanswers = 1;
            $wrapped->layout = qtype_multichoice_base::LAYOUT_DROPDOWN;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE_REGULAR2])) {
            qtype_multianswer2_initialise_multichoice_subquestion($wrapped);
            $wrapped->shuffleanswers = 0;
            $wrapped->layout = qtype_multichoice_base::LAYOUT_VERTICAL;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE_REGULAR_SHUFFLED2])) {
            qtype_multianswer2_initialise_multichoice_subquestion($wrapped);
            $wrapped->shuffleanswers = 1;
            $wrapped->layout = qtype_multichoice_base::LAYOUT_VERTICAL;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE_HORIZONTAL2])) {
            qtype_multianswer2_initialise_multichoice_subquestion($wrapped);
            $wrapped->shuffleanswers = 0;
            $wrapped->layout = qtype_multichoice_base::LAYOUT_HORIZONTAL;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE_HORIZONTAL_SHUFFLED2])) {
            qtype_multianswer2_initialise_multichoice_subquestion($wrapped);
            $wrapped->shuffleanswers = 1;
            $wrapped->layout = qtype_multichoice_base::LAYOUT_HORIZONTAL;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTIRESPONSE2])) {
            qtype_multianswer2_initialise_multichoice_subquestion($wrapped);
            $wrapped->single = 0;
            $wrapped->shuffleanswers = 0;
            $wrapped->layout = qtype_multichoice_base::LAYOUT_VERTICAL;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTIRESPONSE_HORIZONTAL2])) {
            qtype_multianswer2_initialise_multichoice_subquestion($wrapped);
            $wrapped->single = 0;
            $wrapped->shuffleanswers = 0;
            $wrapped->layout = qtype_multichoice_base::LAYOUT_HORIZONTAL;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTIRESPONSE_SHUFFLED2])) {
            qtype_multianswer2_initialise_multichoice_subquestion($wrapped);
            $wrapped->single = 0;
            $wrapped->shuffleanswers = 1;
            $wrapped->layout = qtype_multichoice_base::LAYOUT_VERTICAL;
        } else if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTIRESPONSE_HORIZONTAL_SHUFFLED2])) {
            qtype_multianswer2_initialise_multichoice_subquestion($wrapped);
            $wrapped->single = 0;
            $wrapped->shuffleanswers = 1;
            $wrapped->layout = qtype_multichoice_base::LAYOUT_HORIZONTAL;
        ...
        $remainingalts = $answerregs[ANSWER_REGEX_ALTERNATIVES2];
        while (preg_match('/~?'.ANSWER_ALTERNATIVE_REGEX2.'/s', $remainingalts, $altregs)) {
            if ('=' == $altregs[ANSWER_ALTERNATIVE_REGEX_FRACTION2]) {
                $wrapped->fraction["{$answerindex}"] = '1';
            } else if ($percentile = $altregs[ANSWER_ALTERNATIVE_REGEX_PERCENTILE_FRACTION2]) {
                $wrapped->fraction["{$answerindex}"] = .01 * $percentile;
                $hasspecificfraction = true;
            } else {
                $wrapped->fraction["{$answerindex}"] = '0';
            }
            if (isset($altregs[ANSWER_ALTERNATIVE_REGEX_FEEDBACK2])) {
                $feedback = html_entity_decode(
                        $altregs[ANSWER_ALTERNATIVE_REGEX_FEEDBACK2], ENT_QUOTES, 'UTF-8');
            ...
            if (!empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_NUMERICAL2])
                    && preg_match('~'.NUMERICAL_ALTERNATIVE_REGEX2.'~s',
                            $altregs[ANSWER_ALTERNATIVE_REGEX_ANSWER2], $numregs)) {
                $wrapped->answer[] = $numregs[NUMERICAL_CORRECT_ANSWER2];
                if (array_key_exists(NUMERICAL_ABS_ERROR_MARGIN2, $numregs)) {
                    $wrapped->tolerance["{$answerindex}"] =
                    $numregs[NUMERICAL_ABS_ERROR_MARGIN2];
                } else {
                ...
                $answer = html_entity_decode(
                        $altregs[ANSWER_ALTERNATIVE_REGEX_ANSWER2], ENT_QUOTES, 'UTF-8');
