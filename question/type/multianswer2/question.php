...
/**
 * multianswer2 question definition class.
 *
 * @package    qtype
...
require_once($CFG->dirroot . '/question/type/shortanswer2/question.php');
...
/**
 * Represents a multianswer2 question.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswer2_question extends question_graded_automatically_with_countback {
    ...
    // public function summarise_response() {
    //     $answers = array();
    //     foreach ($this->subquestions as $i => $subq) {
    //         $answers[] = $subq->summarise_response();
    //     }
    //     return $answers;
    // }
    public function get_answers() {
        $answers = array();
        foreach ($this->subquestions as $i => $subq) {
            $answers[] = $subq->get_answers();
        }
        return $answers;
    }
    public function is_duplicate_answer($answers){
        $answers = [];
        foreach ($this->subquestions as $i => $subq) {
            if(in_array($subq->summarise_response(), $answers)){
    ...
    public function get_validation_error(array $response) {
        ...
        return get_string('pleaseananswerallparts', 'qtype_multianswer2');
    }
    ...
    public function grade_response(array $response) {
        ...
        $correct_answers = $this->get_answers();
        $questions = array();
        $answers = array();  
        foreach($response as $question=>$answer){
            if(strpos($question, 'duplicate')){
                unset($response[$question]);
            } elseif(strpos($question, 'userinput')) {
                unset($response[$question]);
            } else {
                $questions[] = $question;
                $answers[] = $answer;
        ...
        foreach($answers as $i=>$answer){
            if(count($correct_answers[$i])>1){
                if(in_array($answer, $tempanswers)){
                    $answers[$i] = null;
                }
                $tempanswers[] = $answer;
            } else {
    ...
    public function get_commutable_question() {
        $commutable_questions = [];
        foreach ($this->subquestons as $i => $subq) {
            foreach ($subq->answers as $key => $value) {
                $value = (array)$value;
                $valuetemp = [];
                $anstemp = [];
                $ans = [];
                foreach ($value as $i => $val){
                    array_push($ans, $val['answer']);
                }
                
                if(array_diff($ans, $anstemp)) {
                    
                } else {
                    array_push($commutable_questions, $value['id']);
        ...
        if(empty($commutable_questions) ){
            return $commutable_questions;
        } else {
            return false;
        }
    }
    ...
    public function summarise_response(array $response) {
        ...
        foreach ($this->subquestions as $i => $subq) {
            ...
            $summary[] = get_string('subqresponse', 'qtype_multianswer2', $a);
        }
