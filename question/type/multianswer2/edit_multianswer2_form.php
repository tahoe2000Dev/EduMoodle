 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class qtype_multianswer2_edit_form extends question_edit_form {

    // The variable $questiondisplay will contain the qtype_multianswer2_extract_question from
    // the questiontext.
    public $questiondisplay;
    // The variable $savedquestiondisplay will contain the qtype_multianswer2_extract_question
    ...
    protected function definition_inner($mform) {
        ...
        if ($questiontext = optional_param_array('questiontext', false, PARAM_RAW)) {
            $this->questiondisplay = fullclone(qtype_multianswer2_extract_question($questiontext));
        ...
        $mform->addElement('submit', 'analyzequestion',
                get_string('decodeverifyquestiontext', 'qtype_multianswer2'));
        ...
        if ($this->reload) {
            for ($sub = 1; $sub <= $countsubquestions; $sub++) {
                ...
                if (isset($this->savedquestiondisplay->options->questions[$sub]->qtype) &&
                    ...
                    $storemess = ' ' . html_writer::tag('span', get_string(
                            'storedqtype', 'qtype_multianswer2', question_bank::get_qtype_name(
                                    $this->savedquestiondisplay->options->questions[$sub]->qtype)),
                            array('class' => 'error'));
                }
                ...
                $mform->addElement('static', 'sub_'.$sub.'_questiontext',
                        get_string('questiondefinition', 'qtype_multianswer2'));
                ...
                $mform->setDefault('sub_'.$sub.'_defaultmark',
                 ...
                 if ($this->questiondisplay->options->questions[$sub]->qtype == 'shortanswer2') {
                    $mform->addElement('static', 'sub_'.$sub.'_usecase',
                            get_string('casesensitive', 'qtype_shortanswer2'));
                }
                ...
                if ($this->questiondisplay->options->questions[$sub]->qtype == 'multichoice') {
                    $mform->addElement('static', 'sub_'.$sub.'_layout',
                            get_string('layout', 'qtype_multianswer2'));
            ...
            if (($this->negativediff > 0) ||$this->qtypechange ||
                ...
                $mform->addElement('header', 'additemhdr',
                        get_string('warningquestionmodified', 'qtype_multianswer2'));
            }
            if ($this->negativediff > 0) {
                $mform->addElement('static', 'alert1', "<strong>".
                        get_string('questiondeleted', 'qtype_multianswer2')."</strong>",
                        get_string('questionsless', 'qtype_multianswer2', $this->negativediff));
            }
            if ($this->qtypechange) {
                $mform->addElement('static', 'alert1', "<strong>".
                        get_string('questiontypechanged', 'qtype_multianswer2')."</strong>",
                        get_string('questiontypechangedcomment', 'qtype_multianswer2'));
            }
        }
        if ($this->usedinquiz) {
            if ($this->negativediff < 0) {
                ...
                $mform->addElement('static', 'alert1', "<strong>".
                        get_string('questionsadded', 'qtype_multianswer2')."</strong>",
                        "<strong>".get_string('questionsmore', 'qtype_multianswer2', $diff).
                        "</strong>");
            }
            $a = new stdClass();
            ...
            $mform->addElement('header', 'additemhdr2',
                    get_string('questionusedinquiz', 'qtype_multianswer2', $a));
            $mform->addElement('static', 'alertas',
                    get_string('youshouldnot', 'qtype_multianswer2'));
        }
        if (($this->negativediff > 0 || $this->usedinquiz &&
                ($this->negativediff > 0 || $this->negativediff < 0 || $this->qtypechange)) &&
                        $this->reload) {
            $mform->addElement('header', 'additemhdr',
                    get_string('questionsaveasedited', 'qtype_multianswer2'));
            $mform->addElement('checkbox', 'confirm', '',
                    get_string('confirmquestionsaveasedited', 'qtype_multianswer2'));
    ...
    public function set_data($question) {
        ...
        if (isset($question->id) and $question->id and $question->qtype &&
            ...
            foreach ($question->options->questions as $key => $wrapped) {
                if (!empty($wrapped)) {
                    ...
                    // update all multianswer2 questions.
                    if (empty($wrapped->questiontext)) {
                        ...
                        switch ($wrapped->qtype) {
                            case 'multichoice':
                                ...
                                break;
                            case 'shortanswer2':
                                $parsableanswerdef .= 'SHORTANSWER2:';
                                break;
                            case 'shortanswer':
            ...
            if (isset($this->questiondisplay->options->questions)) {
                ...
                if (count($subquestions)) {
                    ...
                    foreach ($subquestions as $subquestion) {
                        ...
                        if ($subquestion->qtype == 'shortanswer2') {
                                ...
                                switch ($subquestion->layout) {
                                    case '0':
                                        $defaultvalues[$prefix.'layout'] =
                                            get_string('layoutselectinline', 'qtype_multianswer2');
                                        break;
                                    case '1':
                                        $defaultvalues[$prefix.'layout'] =
                                            get_string('layoutvertical', 'qtype_multianswer2');
                                        break;
                                    case '2':
                                        $defaultvalues[$prefix.'layout'] =
                                            get_string('layouthorizontal', 'qtype_multianswer2');
                                        break;
                                    default:
                                        $defaultvalues[$prefix.'layout'] =
                                            get_string('layoutundefined', 'qtype_multianswer2');
                                }
                            } else {
                                switch ($subquestion->layout) {
                                    case '1':
                                        $defaultvalues[$prefix.'layout'] =
                                            get_string('layoutmultiple_vertical', 'qtype_multianswer2');
                                        break;
                                    case '2':
                                        $defaultvalues[$prefix.'layout'] =
                                            get_string('layoutmultiple_horizontal', 'qtype_multianswer2');
                                        break;
                                    default:
                                        $defaultvalues[$prefix.'layout'] =
                                            get_string('layoutundefined', 'qtype_multianswer2');
        ...
        $defaultvalues['alertas'] = "<strong>".get_string('questioninquiz', 'qtype_multianswer2').
                "</strong>";
    ...
    public function validation($data, $files) {
        ...
        $questiondisplay = qtype_multianswer2_extract_question($data['questiontext']);
        ...
        if (isset($questiondisplay->options->questions)) {
            ...
            if (count($subquestions)) {
                ...
                $errors['questiontext'] = get_string('questionsmissing', 'qtype_multianswer2');
            }
        }
        ...
        if (($this->negativediff > 0 || $this->usedinquiz &&
            ...
            $errors['confirm'] =
                    get_string('confirmsave', 'qtype_multianswer2', $this->negativediff);
        }
