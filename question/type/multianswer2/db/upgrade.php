 ...
 */
function xmldb_qtype_multianswer2_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2015100201) {
        ...
        // can change over time. This is a simple copy & paste from question/type/multianswer2/questiontype.php
        ...
        $rs = $DB->get_recordset_sql("SELECT q.id, q.category, qma.sequence
                 FROM {question} q
                 JOIN {question_multianswer2} qma ON q.id = qma.question");
        foreach ($rs as $q) {
        ...
        $rs->close();
        // multianswer2 savepoint reached.
        upgrade_plugin_savepoint(true, 2015100201, 'qtype', 'multianswer2');
    }
