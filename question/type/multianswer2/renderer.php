...
/**
 * multianswer2 question renderer classes.
...
require_once($CFG->dirroot . '/question/type/shortanswer2/renderer.php');
require_once($CFG->dirroot . '/question/type/shortanswer/renderer.php');


/**
 * Base class for generating the bits of output common to multianswer2
 * (Cloze) questions.
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswer2_renderer extends qtype_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa,
        ...
        foreach ($question->textfragments as $i => $fragment) {
            if ($i > 0) {
                ...
                $token = 'qtypemultianswer2' . $i . 'marker';
        ...
        $this->page->requires->js_init_call('M.qtype_multianswer2.init',
                array('#q' . $qa->get_slot()), false, array(
                    'name'     => 'qtype_multianswer2',
                    'fullpath' => '/question/type/multianswer2/module.js',
        ...
        // $this->page->requires->js('/question/type/multianswer2/script.js');
        return $output;
    }
    ...
    public function subquestion(question_attempt $qa,
        ...
        if ($subtype == 'numerical' || $subtype == 'shortanswer2' || $subtype == 'shortanswer') {
        ...
        $renderer = $this->page->get_renderer('qtype_multianswer2', $subrenderer);
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_multianswer2_subq_renderer_base extends qtype_renderer {
    ...
    protected function feedback_popup(question_graded_automatically $subq,
        ...
        if ($options->rightanswer) {
            if($subq->qtype->name() == 'shortanswer2'){
                $feedback[] = get_string('correctansweris', 'qtype_shortanswer', html_writer::tag('span', $rightanswer, array('class' => 'filter_mathjaxloader_equation')));
            } else {
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswer2_textfield_renderer extends qtype_multianswer2_subq_renderer_base {
    ...
    public function subquestion(question_attempt $qa, question_display_options $options,
        ...
        $duplicatefieldname = $fieldprefix . 'duplicate';

        $response = $qa->get_last_qt_var($fieldname);
        $duplicate = $qa->get_last_qt_var($duplicatefieldname);

        if(!(count($subq->answers) > 1)) {
            $duplicate = 0;
        }

        if ($subq->qtype->name() == 'shortanswer2') {
            $matchinganswer = $subq->get_matching_answer(array('answer' => $response));
        } else if ($subq->qtype->name() == 'shortanswer') {
            $matchinganswer = $subq->get_matching_answer(array('answer' => $response));
        ...
        if ($subq->qtype->name() == 'shortanswer2') {
            $userinputanswer = $qa->get_last_qt_var($fieldprefix . 'userinput');
        }
        ...
        if ($options->correctness) {
            ...
            if ($duplicate) {
                $inputattributes['class'] .= ' ' . $this->feedback_class();
                $feedbackimg = $this->feedback_image();
            }
        }
        ...
        if ($duplicate) {
            if($subq->qtype->name() == 'shortanswer2'){
                $feedbackpopup = $this->feedback_popup($subq, 0,
        ...
        } elseif($subq->qtype->name() == 'shortanswer2'){
            $feedbackpopup = $this->feedback_popup($subq, $matchinganswer->fraction,
                $subq->format_text($matchinganswer->feedback, 1,
                        $qa, 'question', 'answerfeedback', $matchinganswer->id),
                s('\(' . $correctanswer->answer . '\)'), $options);
        } else {
            $feedbackpopup = $this->feedback_popup($subq, $matchinganswer->fraction,
        ...
        if ($subq->qtype->name() == 'shortanswer2') {
            $output .= html_writer::tag('span', '', array('class' => 'matheditor'));
        }
        $output .= html_writer::empty_tag('input', $inputattributes);
        if ($subq->qtype->name() == 'shortanswer2') {
            $output .= html_writer::tag('span', base64_encode(json_encode($subq->answers)), array('id' => $qa->get_qt_field_name('ca'), 'hidden' => 'hidden'));
            $output .= html_writer::tag('input', '', array('type' => 'text', 'name' => $qa->get_qt_field_name($fieldprefix . 'userinput'), 'id' => $qa->get_qt_field_name($fieldprefix . 'userinput'), 'value' => $userinputanswer, "hidden" => "hidden", 'class' => 'form-control'));
        }
        if($duplicate){
            $output .= html_writer::tag('input', '', array('type' => 'text', 'value' => '1', 'name' => $qa->get_qt_field_name($fieldprefix . 'duplicate'), 'id' => $qa->get_qt_field_name($fieldprefix . 'duplicate'), 'hidden' => 'hidden'));
        } else {
            $output .= html_writer::tag('input', '', array('type' => 'text', 'value' => '0', 'name' => $qa->get_qt_field_name($fieldprefix . 'duplicate'), 'id' => $qa->get_qt_field_name($fieldprefix . 'duplicate'), 'hidden' => 'hidden'));
        }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswer2_multichoice_inline_renderer
        extends qtype_multianswer2_subq_renderer_base {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswer2_multichoice_vertical_renderer extends qtype_multianswer2_subq_renderer_base {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswer2_multichoice_horizontal_renderer
        extends qtype_multianswer2_multichoice_vertical_renderer {
...
/**
 * Class qtype_multianswer2_multiresponse_renderer
 *
 * @copyright  2016 Davo Smith, Synergy Learning
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswer2_multiresponse_vertical_renderer extends qtype_multianswer2_subq_renderer_base {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswer2_multiresponse_horizontal_renderer
    extends qtype_multianswer2_multiresponse_vertical_renderer {
