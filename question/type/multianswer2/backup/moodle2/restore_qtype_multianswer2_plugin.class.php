...
require_once($CFG->dirroot . '/question/type/multianswer2/questiontype.php');
/**
 * restore plugin class that provides the necessary information
 * needed to restore one multianswer2 qtype plugin
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_qtype_multianswer2_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        ...
        // Add own qtype stuff.
        $elename = 'multianswer2';
        $elepath = $this->get_pathfor('/multianswer2');
    ...
    /**
     * Process the qtype/multianswer2 element
     */
    public function process_multianswer2($data) {
        global $DB;
        ...
        // question_multianswer2 too.
        if ($questioncreated) {
            // Adjust some columns.
            $data->question = $newquestionid;
            // Note: multianswer2->sequence is a list of question->id values. We aren't
            ...
            // Insert record.
            $newitemid = $DB->insert_record('question_multianswer2', $data);
            // Create mapping (need it for after_execute recode of sequence).
            $this->set_mapping('question_multianswer2', $oldid, $newitemid);
        }
    }
     ...
     *
     * For multianswer2 qtype we use it to restore the sequence column,
    ...
    public function after_execute_question() {
        ...
        // the created question_multianswer2 sequences (list of question ids).
        $rs = $DB->get_recordset_sql("
                SELECT qma.id, qma.sequence
                  FROM {question_multianswer2} qma
        ...
        foreach ($rs as $rec) {
            ...
            if (substr_count($rec->sequence, ',') + 1 != count($sequencearr)) {
                $this->task->log('Invalid sequence found in restored multianswer2 question ' . $rec->id, backup::LOG_WARNING);
            }
            ...
            $DB->set_field('question_multianswer2', 'sequence', $sequence,
                    array('id' => $rec->id));
            if (!empty($sequence)) {
     ...
     * recoded pointing to all the restored stuff for multianswer2 questions
     *
     * answer is one comma separated list of hypen separated pairs
     * containing sequence (pointing to questions sequence in question_multianswer2)
