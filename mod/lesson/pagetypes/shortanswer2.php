...
defined('MOODLE_INTERNAL') || die();
 ...
 /** Short answer 2 question type */
define("LESSON_PAGE_SHORTANSWER2",   "4");

class lesson_page_type_shortanswer2 extends lesson_page {

    protected $type = lesson_page::TYPE_QUESTION;
    protected $typeidstring = 'shortanswer2';
    protected $typeid = LESSON_PAGE_SHORTANSWER2;
    ...
    public function display($renderer, $attempt) {
        ...
        $hasattempt = false;
        $attrs = array('name'=>'answer', 'type'=>'text', 'id'=>'id_answer', 'size'=>'50', 'maxlength'=>'200', 'class'=>'form-control', 'hidden'=>'hidden');
        ...
        $question = '';
        if ($placeholder) {
            $question .= $contentsparts[0];
            $question .= $matheditor;
            $question .= html_writer::empty_tag('input', $attrs);
            $question .= $correctanswer;
            $question .= $contentsparts[1];
            
        } else {
            $question .= $contents;
            $question .= html_writer::tag('lable', get_string('youranswer', 'lesson'));
            $question .= $matheditor;
            $question .= html_writer::tag('input', '', $attrs);
            $question .= $correctanswer;
        }

        $questionfield = html_writer::tag('div', html_writer::tag('div', '', array('class'=>'col-md-3')) . html_writer::tag('div', $question, array('class'=>'col-md-9 form-inline felement')), array('class'=>'row fitem'));
        ...
        if ($hasattempt) {
            ...
            $control = html_writer::empty_tag('input', array('value'=>get_string("submit", "lesson"), 'type'=>'submit', 'class'=>'btn btn-primary',
                                                            'id'=>'id_submitbutton', 'name'=>'submitbutton'
                                                        ));
        }
        $formcontrol = html_writer::tag('div', html_writer::tag('div', '', array('class'=>'col-md-3')) . html_writer::tag('div', $control, array('class'=>'col-md-9 form-inline felement')), array('class'=>'row fitem'));
...
class lesson_add_page_form_shortanswer2 extends lesson_add_page_form_base {
    public $qtype = 'shortanswer2';
    public $qtypestring = 'shortanswer2';
...
class lesson_display_answer_form_shortanswer2 extends moodleform {
